...
#$ -pe shmem 8
#$ -q short.qe
#$ -o logs/klfda_cv.o.log
#$ -e logs/klfda_cv.e.log

# experiment in choosing the dimensionality of klfda reduced space
...
source("scripts/fun_klfda.R"); source("scripts/fun_kernel.R")

N = 6000
set.seed(10)
data.cv = sample(1:ntrain, N)
...
for (foldi in 1:5) {
  ...
  for(dim in dim.lst){
    klfda.res <- klfda(rbfkernel(data.X.train.cv, sigma = gaussian_width), 
                       data.y.cv, dim,
                       knn = 7 ,metric = "orthonormalized")
    X.train.klfda <- klfda.res$Z
    X.test.klfda <- rbfkernel(data.X.test.cv, data.X.train.cv, 
                              sigma = gaussian_width) %*% klfda.res$T
    knn.res = knn1(X.train.klfda, X.test.klfda, data.y.cv)
...
write.table(mis.class.error, file.path(configs$results.dir,"_klfda.csv"))
